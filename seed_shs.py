...
with app.app_context():
    for category_name, courses in courses_data.items():
        ...
        for course_name, subjects_list in courses.items():
            ...
            if category_name in ["JHS", "SHS"]:
                ...
                level_count = 4  # Example Tertiary levels/years

            levels = []
            for i in range(1, level_count + 1):
                level = Level(name=f"{course_name} Year {i}" if category_name=="Tertiary" else f"{category_name} {i}", order=i, course=course)
                levels.append(level)
    ...
    print("✅ All West African courses, levels, and subjects seeded successfully!")
